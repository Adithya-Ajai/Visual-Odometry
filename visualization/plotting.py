...
from bokeh.models import Panel, Tabs, Arrow, NormalHead
...
def visualize_paths(gt_path, pred_path, html_tile="", title="VO exercises", file_out="plot.html"):
    ...
    # Calculate directions for ground truth and predicted paths
    gt_dx = np.diff(gt_x, prepend=gt_x[0])
    gt_dy = np.diff(gt_y, prepend=gt_y[0])
    pred_dx = np.diff(pred_x, prepend=pred_x[0])
    pred_dy = np.diff(pred_y, prepend=pred_y[0])
    ...
    fig1 = figure(title="Paths with Directions", tools=tools, match_aspect=True, width_policy="max", toolbar_location="above",
    ...
    # Add quivers (arrows) for directions
    fig1.add_layout(Arrow(end=NormalHead(size=10, fill_color="blue"), x_start=gt_x[:-1], y_start=gt_y[:-1],
                          x_end=gt_x[:-1] + gt_dx[:-1], y_end=gt_y[:-1] + gt_dy[:-1], line_color="blue"))
    fig1.add_layout(Arrow(end=NormalHead(size=10, fill_color="green"), x_start=pred_x[:-1], y_start=pred_y[:-1],
                          x_end=pred_x[:-1] + pred_dx[:-1], y_end=pred_y[:-1] + pred_dy[:-1], line_color="green"))
